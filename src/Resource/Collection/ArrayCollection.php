...
namespace Solital\Core\Resource\Collection;

use Solital\Core\Resource\Collection\Helper\Arr;
use Solital\Core\Resource\Collection\Helper\Helpers;
use Solital\Core\Resource\Collection\Helper\Jsonable;
    ...
    \JsonSerializable
{
    ...
    public function containsStrict($key, $value = null)
    {
        if (func_num_args() === 2) {
            return $this->contains(function ($item) use ($key, $value) {
                return Helpers::dataGet($item, $key) === $value;
            });
        }
     ...
     * @return static
     */
    public function exceptMultiple($keys)
    {
        ...
        return new static(Arr::exceptMultiple($this->items, $keys));
    }
    ...
    public function whereIn($key, $values, $strict = false)
    {
        ...
        return $this->filter(function ($item) use ($key, $values, $strict) {
            return in_array(Helpers::dataGet($item, $key), $values, $strict);
        });
    }
    ...
    public function whereNotBetween($key, $values)
    {
        return $this->filter(function ($item) use ($key, $values) {
            return Helpers::dataGet($item, $key) < reset($values) || Helpers::dataGet($item, $key) > end($values);
        });
    }
    ...
    protected function valueRetriever($value)
    {
        ...
        return function ($item) use ($value) {
            return Helpers::dataGet($item, $value);
        };
    }
